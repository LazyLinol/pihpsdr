...
void set_deviation(RECEIVER *rx) { SetRXAFMDeviation(rx->id, (double)rx->deviation); }
...
RECEIVER *create_receiver(int id, int buffer_size, int fft_size, int pixels, int fps, int width, int height) {
  ...
  fprintf(stderr, "create_receiver: OpenChannel id=%d buffer_size=%d fft_size=%d sample_rate=%d\n", rx->id, rx->buffer_size,
      2048, // rx->fft_size,
      rx->sample_rate);
  OpenChannel(rx->id, rx->buffer_size,
      2048, // rx->fft_size,
      rx->sample_rate,
      48000, // dsp rate
      48000, // output rate

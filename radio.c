...
void start_radio() {
  ...
  switch (radio->protocol) {
    ...
    case NEW_PROTOCOL:
...
#endif
        sprintf(text, "Starting %s (%s v%d.%d)", radio->name, radio->protocol == ORIGINAL_PROTOCOL ? "Protocol 1" : "Protocol 2", radio->software_version / 10, radio->software_version % 10);
        break;
      }
      //}
      ...
      sprintf(text, "piHPSDR: %s (%s v%d.%d) %s (%02X:%02X:%02X:%02X:%02X:%02X) on %s", radio->name, radio->protocol == ORIGINAL_PROTOCOL ? "Protocol 1" : "Protocol 2", radio->software_version / 10, radio->software_version % 10, inet_ntoa(radio->info.network.address.sin_addr), radio->info.network.mac_address[0], radio->info.network.mac_address[1], radio->info.network.mac_address[2], radio->info.network.mac_address[3], radio->info.network.mac_address[4], radio->info.network.mac_address[5], radio->info.network.interface_name);
...
#endif
            default:
              sprintf(property_path, "%02X-%02X-%02X-%02X-%02X-%02X.props", radio->info.network.mac_address[0], radio->info.network.mac_address[1], radio->info.network.mac_address[2], radio->info.network.mac_address[3], radio->info.network.mac_address[4], radio->info.network.mac_address[5]);
              break;
          }
          break;
#ifdef LIMESDR
  ...
  void vox_changed(int state) { setVox(state); }
  ...
  // DL1YCF: because of the new CW algorithm,
  //         this function is no longer used
  void radio_cw_key(int state) { SetTXAPostGenRun(transmitter->id, state); }

  int isTransmitting() { return ptt | mox | vox | tune; }
  ...
  sem_wait(&property_sem);
    ...
    */
    ...
    if (value)
      e1_encoder_action = atoi(value);
    value = getProperty("e2_encoder_action");
    if (value)
      e2_encoder_action = atoi(value);
    value = getProperty("e3_encoder_action");
    if (value)
      e3_encoder_action = atoi(value);
#endif
    ...
    if (value)
      adc_attenuation[0] = atoi(value);
    value = getProperty("adc_1_attenuation");
    if (value)
      adc_attenuation[1] = atoi(value);

    value = getProperty("rx1_gain_slider");
    if (value)
      rx_gain_slider[0] = atoi(value);
    value = getProperty("rx2_gain_slider");
    if (value)
      rx_gain_slider[1] = atoi(value);

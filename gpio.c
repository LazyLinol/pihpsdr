...
char *encoder_string[] = {"AF GAIN", "AGC GAIN",
#ifdef RADIOBERRY
    "RX-GAIN",
#else
    "ATTENUATION",
#endif
    "MIC GAIN", "DRIVE", "RIT", "CW SPEED", "CW FREQUENCY", "PANADAPTER HIGH", "PANADAPTER LOW", "SQUELCH", "COMP"};
...
static int vfo_function_released(void *data) { return 0; }
...
static void e1FunctionAlert() {
  ...
  if (level == 0) {
    if (running)
      g_idle_add(e_function_pressed, (gpointer)1);
  }
}
...
static void e2FunctionAlert() {
  ...
  if (level == 0) {
    if (running)
      g_idle_add(e_function_pressed, (gpointer)2);
  }
}
...
static void e3FunctionAlert() {
  ...
  if (level == 0) {
    if (running)
      g_idle_add(e_function_pressed, (gpointer)3);
  }
}
...
static void e4FunctionAlert() {
  ...
  if (level == 0) {
    if (running)
      g_idle_add(e_function_pressed, (gpointer)4);
  }
}
#endif
...
static void functionAlert() {
  ...
  if (t - function_debounce > settle_time) {
    ...
    if (level == 0) {
      if (running)
        g_idle_add(function_pressed, NULL);
    }
    function_debounce = t;
  }
}
...
static void vfoFunctionAlert() {
  ...
  if (t - vfo_debounce > settle_time) {
    ...
    if (level == 0) {
      if (running)
        g_idle_add(vfo_function_pressed, NULL);
    } else {
      if (running)
        g_idle_add(vfo_function_released, NULL);
    }
    vfo_debounce = t;
  }
}
...
static void vfoEncoderA() { vfoEncoderInt(VFO_ENCODER_A); }

static void vfoEncoderB() { vfoEncoderInt(VFO_ENCODER_B); }
...
static void e1EncoderA() { e1EncoderInterrupt(E1_ENCODER_A); }

static void e1EncoderB() { e1EncoderInterrupt(E1_ENCODER_B); }
...
static void e2EncoderA() { e2EncoderInterrupt(E2_ENCODER_A); }

static void e2EncoderB() { e2EncoderInterrupt(E2_ENCODER_B); }
...
static void e3EncoderA() { e3EncoderInterrupt(E3_ENCODER_A); }

static void e3EncoderB() { e3EncoderInterrupt(E3_ENCODER_B); }
...
static void e4EncoderA() { e4EncoderInterrupt(E4_ENCODER_A); }

static void e4EncoderB() { e4EncoderInterrupt(E4_ENCODER_B); }
#endif
...
int e3_function_get_state() { return e3Function; }
...
int function_get_state() { return function_state; }

int band_get_state() { return band_state; }

int bandstack_get_state() { return bandstack_state; }

int mode_get_state() { return mode_state; }

int filter_get_state() { return filter_state; }

int noise_get_state() { return noise_state; }

int agc_get_state() { return agc_state; }

int mox_get_state() { return mox_state; }

int lock_get_state() { return lock_state; }

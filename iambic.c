...
void keyer_event(int left, int state) {
  ...
  if (left) {
    ...
    kcwl = state;
    if (state)
      *kmeml = 1; // trigger dot/dash memory
  } else {
    // right paddle hit or released
    kcwr = state;
    if (state)
      *kmemr = 1; // trigger dot/dash memory
  }
  if (state) {
#ifdef __APPLE__
    sem_post(cw_event);
...
static void *keyer_thread(void *arg) {
  ...
  while (running) {
    ...
    while (key_state != EXITLOOP || cwvox > 0) {
      ...
      if (cwvox > 0 && key_state != EXITLOOP && key_state != CHECK)
        cwvox = (int)cw_keyer_hang_time;
        ...
        case DOTDELAY:
          ...
          if (kdelay > dot_length) {
            ...
            if (cw_keyer_mode == KEYER_STRAIGHT) {
              ...
              //
              if (cw_keyer_mode == KEYER_MODE_A && !*kdot && !*kdash)
                dash_held = 0;

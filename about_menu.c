  ...
  switch (radio->protocol) {
    ...
    case NEW_PROTOCOL:
      ...
      if (d->device == DEVICE_OZY) {
        ...
        sprintf(text, "%s\nDevice Mac Address: %02X:%02X:%02X:%02X:%02X:%02X", text, radio->info.network.mac_address[0], radio->info.network.mac_address[1], radio->info.network.mac_address[2], radio->info.network.mac_address[3], radio->info.network.mac_address[4], radio->info.network.mac_address[5]);

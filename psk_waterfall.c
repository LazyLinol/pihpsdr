...
GtkWidget *psk_waterfall_init(int width, int height) {
  ...
  g_signal_connect(waterfall, "draw", G_CALLBACK(waterfall_draw_cb), NULL);
  g_signal_connect(waterfall, "configure-event", G_CALLBACK(waterfall_configure_event_cb), NULL);

  //pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, FALSE, 8, width, height);

  /* Event signals */
  g_signal_connect(waterfall, "motion-notify-event", G_CALLBACK(waterfall_motion_notify_event_cb), NULL);
  g_signal_connect(waterfall, "button-press-event", G_CALLBACK(waterfall_button_press_event_cb), NULL);
  g_signal_connect(waterfall, "button-release-event", G_CALLBACK(waterfall_button_release_event_cb), NULL);
  g_signal_connect(waterfall, "scroll_event", G_CALLBACK(waterfall_scroll_event_cb), NULL);

...
void parse_cmd(char *cmd_input, int len, int client_sock) {
  ...
  // PiHPSDR - ZZAU - Reserved for Audio UDP stream start/stop
    ...
    if (len > 2) {
      ...
      if ((ctcss_tone > 0) && (ctcss_tone <= 39)) {
        switch (ctcss_tone) {
          case 1:
            transmitter->ctcss_frequency = (double)67.0;
            break;
          case 2:
            transmitter->ctcss_frequency = (double)71.9;
            break;
          case 3:
            transmitter->ctcss_frequency = (double)74.4;
            break;
          case 4:
            transmitter->ctcss_frequency = (double)77.0;
            break;
          case 5:
            transmitter->ctcss_frequency = (double)79.7;
            break;
          case 6:
            transmitter->ctcss_frequency = (double)82.5;
            break;
          case 7:
            transmitter->ctcss_frequency = (double)85.4;
            break;
          case 8:
            transmitter->ctcss_frequency = (double)88.5;
            break;
          case 9:
            transmitter->ctcss_frequency = (double)91.5;
            break;
          case 10:
            transmitter->ctcss_frequency = (double)94.8;
            break;
          case 11:
            transmitter->ctcss_frequency = (double)97.4;
            break;
          case 12:
            transmitter->ctcss_frequency = (double)100.0;
            break;
          case 13:
            transmitter->ctcss_frequency = (double)103.5;
            break;
          case 14:
            transmitter->ctcss_frequency = (double)107.2;
            break;
          case 15:
            transmitter->ctcss_frequency = (double)110.9;
            break;
          case 16:
            transmitter->ctcss_frequency = (double)114.8;
            break;
          case 17:
            transmitter->ctcss_frequency = (double)118.8;
            break;
          case 18:
            transmitter->ctcss_frequency = (double)123.0;
            break;
          case 19:
            transmitter->ctcss_frequency = (double)127.3;
            break;
          case 20:
            transmitter->ctcss_frequency = (double)131.8;
            break;
          case 21:
            transmitter->ctcss_frequency = (double)136.5;
            break;
          case 22:
            transmitter->ctcss_frequency = (double)141.3;
            break;
          case 23:
            transmitter->ctcss_frequency = (double)146.2;
            break;
          case 24:
            transmitter->ctcss_frequency = (double)151.4;
            break;
          case 25:
            transmitter->ctcss_frequency = (double)156.7;
            break;
          case 26:
            transmitter->ctcss_frequency = (double)162.2;
            break;
          case 27:
            transmitter->ctcss_frequency = (double)167.9;
            break;
          case 28:
            transmitter->ctcss_frequency = (double)173.8;
            break;
          case 29:
            transmitter->ctcss_frequency = (double)179.9;
            break;
          case 30:
            transmitter->ctcss_frequency = (double)186.2;
            break;
          case 31:
            transmitter->ctcss_frequency = (double)192.8;
            break;
          case 32:
            transmitter->ctcss_frequency = (double)203.5;
            break;
          case 33:
            transmitter->ctcss_frequency = (double)210.7;
            break;
          case 34:
            transmitter->ctcss_frequency = (double)218.1;
            break;
          case 35:
            transmitter->ctcss_frequency = (double)225.7;
            break;
          case 36:
            transmitter->ctcss_frequency = (double)233.6;
            break;
          case 37:
            transmitter->ctcss_frequency = (double)241.8;
            break;
          case 38:
            transmitter->ctcss_frequency = (double)250.3;
            break;
          case 39:
            transmitter->ctcss_frequency = (double)1750.0;
            break;
        }
  ...
  // ZZCS - Keyer Speed implemented within "KS" command
    ...
    sprintf(msg, "ZZDU%1d%1d%1d%1d%1d%1d%1d%1d%1d%06d%1d%03d%1c%1d%1d%1d%1c%06lld%1d%011lld%011lld%c%03d%1d%1d%1d%1d;",
        ...
        smeter >= 0 ? '+' : '-', abs(smeter), active_receiver->nr, active_receiver->nr2, active_receiver->anf, active_receiver->snb);
    send_resp(client_sock, msg);
  }
    ...
    if (len <= 2) {
      sprintf(msg, "ZZEA%1d%1c%03d%1c%03d%1c%03d%1c%03d;", enable_rx_equalizer, rx_equalizer[0] >= 0 ? '+' : '-', abs((int)rx_equalizer[0]), rx_equalizer[1] >= 0 ? '+' : '-', abs((int)rx_equalizer[1]), rx_equalizer[2] >= 0 ? '+' : '-', abs((int)rx_equalizer[2]), rx_equalizer[3] >= 0 ? '+' : '-', abs((int)rx_equalizer[3]));
      send_resp(client_sock, msg);
    ...
    if (len <= 2) {
      sprintf(msg, "ZZEB%1d%1c%03d%1c%03d%1c%03d%1c%03d;", enable_tx_equalizer, tx_equalizer[0] >= 0 ? '+' : '-', abs((int)tx_equalizer[0]), tx_equalizer[1] >= 0 ? '+' : '-', abs((int)tx_equalizer[1]), tx_equalizer[2] >= 0 ? '+' : '-', abs((int)tx_equalizer[2]), tx_equalizer[3] >= 0 ? '+' : '-', abs((int)tx_equalizer[3]));
      send_resp(client_sock, msg);
...
// SDW1
    ...
    if (len > 2) { // Set Mode
      ...
      switch (vfo[active_receiver->id].mode) {
        ...
        default:
#ifdef RIGCTL_DEBUG
          fprintf(stderr, "RIGCTL: MD command doesn't support %d\n", vfo[active_receiver->id].mode);
#endif
          break;
      }
  ...
  /* Not Supported */
...
#endif
        if (lcl_rate != 48000L && lcl_rate != 96000L && lcl_rate != 192000L && lcl_rate != 384000L) {
    ...
    if (len <= 2) {
      ...
      if ((lcl_attenuation >= 0) && (lcl_attenuation <= 99)) {
        ...
        if (local_cw_breakin > 1000) {
          local_cw_breakin = 1000;
        }
        if (len <= 2) {
        ...
        if (zzid_flag == 0) {
          sprintf(msg, "SM%1c%04d;", cmd_input[2], new_level);
        } else {
          sprintf(msg, "ZZSM%1c%04d;", cmd_input[2], new_level);
        }
        send_resp(client_sock, msg);
      }
              ...
              case modeAM:
                switch (coded_step_val) {
                  ...
                  case 9:
                    ...
                    break;
                  default:
                    break; // No change if not a valid number
                }
              default:
                break; // No change if not a valid number
            }
        ...
        if (len <= 3) {
          ...
          if ((work_int >= 0) && (work_int <= 7)) {
            ...
            lcl_buf[15] = '\0';
            sprintf(msg, "ZZUA%1d%1d%s%011lld%011lld%011lld;", work_int, xvtr->disablePA, lcl_buf, xvtr->frequencyMin, xvtr->frequencyMax, xvtr->frequencyLO);
            send_resp(client_sock, msg);
          }
        } else if (len == 52) {
    ...
    int set_band(gpointer data) {
      ...
      if ((vfo[active_receiver->id].ctun == 1) && ((vfo[active_receiver->id].ctun_frequency + (active_receiver->sample_rate / 2) < new_freq) || (vfo[active_receiver->id].ctun_frequency - (active_receiver->sample_rate / 2) > new_freq))) {

...
void transmitter_set_deviation(TRANSMITTER *tx) { SetTXAFMDeviation(tx->id, (double)tx->deviation); }

void transmitter_set_am_carrier_level(TRANSMITTER *tx) { SetTXAAMCarrierLevel(tx->id, tx->am_carrier_level); }
...
void reconfigure_transmitter(TRANSMITTER *tx, int height) { gtk_widget_set_size_request(tx->panadapter, tx->width, height); }
...
void transmitter_restore_state(TRANSMITTER *tx) {
  ...
  value = getProperty(name);
  if (value)
    strcpy(tx->freedv_text_data, value);
#endif
  sprintf(name, "transmitter.%d.drive", tx->id);
  value = getProperty(name);
  if (value)
...
void tx_set_pre_emphasize(TRANSMITTER *tx, int state) { SetTXAFMEmphPosition(tx->id, state); }
...
void tx_set_ps_sample_rate(TRANSMITTER *tx, int rate) { SetPSFeedbackRate(tx->id, rate); }
#endif

...
static char *mode_string[] = {"1600", "700", "700B", "2400A", "2400B", "800XA", "700C"};

char *freedv_get_mode_string() { return mode_string[freedv_mode]; }
...
void freedv_restore_state() {
  ...
  if (value)
    freedv_mode = atoi(value);
  sprintf(name, "freedv_audio_gain");
  value = getProperty(name);
  if (value)
    freedv_audio_gain = atof(value);
}
...
void freedv_reset_tx_text_index() { transmitter->freedv_text_index = 0; }

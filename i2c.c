...
static void frequencyStep(int pos) { vfo_step(pos); }
...
void i2c_init() {
  ...
  // setup i2c
  if (write_byte_data(ADDRESS_1, 0x0A, 0x44) < 0)
    return;
  if (write_byte_data(ADDRESS_1, 0x0B, 0x44) < 0)
    return;

  // disable interrupt
  if (write_byte_data(ADDRESS_1, 0x04, 0x00) < 0)
    return;
  if (write_byte_data(ADDRESS_1, 0x05, 0x00) < 0)
    return;

  // clear defaults
  if (write_byte_data(ADDRESS_1, 0x06, 0x00) < 0)
    return;
  if (write_byte_data(ADDRESS_1, 0x07, 0x00) < 0)
    return;

  // OLAT
  if (write_byte_data(ADDRESS_1, 0x14, 0x00) < 0)
    return;
  if (write_byte_data(ADDRESS_1, 0x15, 0x00) < 0)
    return;

  // set GPIOA for pullups
  if (write_byte_data(ADDRESS_1, 0x0C, 0xFF) < 0)
    return;
  if (write_byte_data(ADDRESS_1, 0x0D, 0xFF) < 0)
    return;

  // reverse polarity
  if (write_byte_data(ADDRESS_1, 0x02, 0xFF) < 0)
    return;
  if (write_byte_data(ADDRESS_1, 0x03, 0xFF) < 0)
    return;

  // set GPIOA/B for input
  if (write_byte_data(ADDRESS_1, 0x00, 0xFF) < 0)
    return;
  if (write_byte_data(ADDRESS_1, 0x01, 0xFF) < 0)
    return;

  // INTCON
  if (write_byte_data(ADDRESS_1, 0x08, 0x00) < 0)
    return;
  if (write_byte_data(ADDRESS_1, 0x09, 0x00) < 0)
    return;

  // setup for an MCP23017 interrupt
  if (write_byte_data(ADDRESS_1, 0x04, 0xFF) < 0)
    return;
  if (write_byte_data(ADDRESS_1, 0x05, 0xFF) < 0)
    return;

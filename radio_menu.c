...
static void vfo_divisor_value_changed_cb(GtkWidget *widget, gpointer data) { vfo_encoder_divisor = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(widget)); }

static void gpio_settle_value_changed_cb(GtkWidget *widget, gpointer data) { settle_time = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(widget)); }
...
static void ptt_cb(GtkWidget *widget, gpointer data) { mic_ptt_enabled = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget)); }
...
static void bias_cb(GtkWidget *widget, gpointer data) { mic_bias_enabled = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget)); }
...
static void sample_rate_cb(GtkWidget *widget, gpointer data) { radio_change_sample_rate((uintptr_t)data); }

static void receivers_cb(GtkWidget *widget, gpointer data) { radio_change_receivers((uintptr_t)data); }
...
static void rit_cb(GtkWidget *widget, gpointer data) { rit_increment = (uintptr_t)data; }

static void ck10mhz_cb(GtkWidget *widget, gpointer data) { atlas_clock_source_10mhz = (uintptr_t)data; }

static void ck128mhz_cb(GtkWidget *widget, gpointer data) { atlas_clock_source_128mhz = atlas_clock_source_128mhz == 1 ? 0 : 1; }

static void micsource_cb(GtkWidget *widget, gpointer data) { atlas_mic_source = atlas_mic_source == 1 ? 0 : 1; }

static void penelopetx_cb(GtkWidget *widget, gpointer data) { atlas_penelope = atlas_penelope == 1 ? 0 : 1; }
...
void radio_menu(GtkWidget *parent) {
  ...
  */
  ...
  if (protocol == ORIGINAL_PROTOCOL || protocol == NEW_PROTOCOL) {
    ...
    if ((protocol == NEW_PROTOCOL && device == NEW_DEVICE_ORION) || (protocol == NEW_PROTOCOL && device == NEW_DEVICE_ORION2) || (protocol == ORIGINAL_PROTOCOL && device == DEVICE_ORION) || (protocol == ORIGINAL_PROTOCOL && device == DEVICE_ORION2)) {
